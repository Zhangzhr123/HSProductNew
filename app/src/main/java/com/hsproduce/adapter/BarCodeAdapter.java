...
public class BarCodeAdapter extends BaseAdapter {
    ...
    public BarCodeAdapter(Context context, List<VreCord> vreCodeList) {
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        convertView = LayoutInflater.from(context).inflate(R.layout.list_barcode_item, null);
        final VreCord vreCord = vreCodeList.get(position);
        //设置页面数据
        if(vreCord.getPlanid() != null){//胎胚规格名称
            ((TextView)convertView.findViewById(R.id.planid)).setText(vreCord.getPlanid());
        }
        if(vreCord.getItnbr() != null){
            ((TextView)convertView.findViewById(R.id.spesc)).setText(vreCord.getItnbr());
        }
        if(vreCord.getItdsc() != null){//胎胚规格名称
